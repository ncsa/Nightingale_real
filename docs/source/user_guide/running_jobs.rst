...
Nightingale uses the `Slurm job control software <https://slurm.schedmd.com/documentation.html>`_ to run jobs.  Slurm commands are *only* available, and will only work, on ng-login01.  If your allocation has batch access, then you can log into the login node.  The `Slurm quick start user guide <https://slurm.schedmd.com/quickstart.html>`_ provides a brief overview of Slurm commands for new users.
