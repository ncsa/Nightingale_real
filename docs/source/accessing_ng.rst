...
where <username> is the username you created for your NCSA identity. After entering the ssh command, you will be prompted to enter your password followed by a prompt to send a push to the Duo app. After you have approved the push, you may then enter the SSH command using the hostname of your login node.
...
The Linux operating system has SSH built-in it. You use the Linux terminal application to connect via SSH.  `PuTTY <http://www.chiark.greenend.org.uk/~sgtatham/putty/>`_ is also available for Linux.
